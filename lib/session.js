 ...
 */ 
Session.prototype.wrapKey = function wrapKey(alg, wkey, key) {
	var hSession = this.handle;
	...
	Debug('C_WrapKey');
	var res = this.cki.C_WrapKey(hSession, pMechanism.ref(), hWrappingKey, hKey, pWrappedKey, pulWrappedKeyLen.ref());
 ...
 */ 
Session.prototype.unwrapKey = function unwrapKey(alg, ukey, obj, data) {
	var hSession = this.handle;
	var pMechanism = MechanismInfo.create(alg);
	...
	Debug('C_UnwrapKey');
	var res = this.cki.C_UnwrapKey(hSession, pMechanism.ref(), hUnwrappingKey, pWrappedKey, ulWrappedKeyLen, pTemplate.ref(), ulAttributeCount, phKey.ref());
